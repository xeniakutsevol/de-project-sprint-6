...
hk_user_id bigint not null CONSTRAINT fk_l_user_group_activity_user REFERENCES XENIAKUTSEVOL_GMAIL_COM__DWH.h_users (hk_user_id),
hk_group_id bigint not null CONSTRAINT fk_l_user_group_activity_group REFERENCES XENIAKUTSEVOL_GMAIL_COM__DWH.h_groups (hk_group_id),
load_dt datetime,
load_src varchar(20)
)
order by load_dt
...
where hash(hu.hk_user_id, hg.hk_group_id) not in (select hk_l_user_group_activity from XENIAKUTSEVOL_GMAIL_COM__DWH.l_user_group_activity);


-- dwh. satellites
drop table if exists XENIAKUTSEVOL_GMAIL_COM__DWH.s_auth_history;
create table XENIAKUTSEVOL_GMAIL_COM__DWH.s_auth_history (
hk_l_user_group_activity bigint not null CONSTRAINT fk_s_auth_history_l_user_group_activity REFERENCES XENIAKUTSEVOL_GMAIL_COM__DWH.l_user_group_activity (hk_l_user_group_activity),
user_id_from int,
event varchar(100),
event_dt timestamp,
load_dt datetime,
...
INSERT INTO XENIAKUTSEVOL_GMAIL_COM__DWH.s_auth_history(hk_l_user_group_activity, user_id_from, event, event_dt, load_dt, load_src)
select distinct
...
left join XENIAKUTSEVOL_GMAIL_COM__DWH.h_groups as hg on gl.group_id = hg.group_id
left join XENIAKUTSEVOL_GMAIL_COM__DWH.h_users as hu on gl.user_id = hu.user_id
left join XENIAKUTSEVOL_GMAIL_COM__DWH.l_user_group_activity as luga on hg.hk_group_id = luga.hk_group_id and hu.hk_user_id = luga.hk_user_id;
...
where hk_user_id in (select distinct hk_user_id from XENIAKUTSEVOL_GMAIL_COM__DWH.l_user_message)
...
from XENIAKUTSEVOL_GMAIL_COM__DWH.l_user_group_activity luga
where luga.hk_l_user_group_activity in (select hk_l_user_group_activity from XENIAKUTSEVOL_GMAIL_COM__DWH.s_auth_history where event='add') 
and luga.hk_group_id in (select hk_group_id from XENIAKUTSEVOL_GMAIL_COM__DWH.h_groups order by registration_dt limit 10)
